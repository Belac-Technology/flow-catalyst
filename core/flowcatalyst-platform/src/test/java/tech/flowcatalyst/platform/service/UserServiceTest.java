...
class UserServiceTest {
    ...
    void createInternalUser_shouldCreateUser_whenAllParametersValid() {
        ...
        // Act
        Principal result = service.createInternalUser(email, password, name, tenantId);
    ...
    @Test
    @DisplayName("createInternalUser should accept null tenantId for anchor domain users")
    void createInternalUser_shouldAcceptNullTenantId_whenAnchorDomainUser() {
        // Arrange
    ...
    void createOrUpdateOidcUser_shouldCreateNewUser_whenUserDoesNotExist() {
        ...
        Principal result = service.createOrUpdateOidcUser(email, name, externalIdpId, tenantId);
    ...
    @Test
    @DisplayName("createOrUpdateOidcUser should accept null tenantId for anchor domain")
    void createOrUpdateOidcUser_shouldAcceptNullTenantId_whenAnchorDomain() {
        // Arrange
        when(principalRepo.findByEmail(anyString())).thenReturn(Optional.empty());
    ...
    @Test
    @DisplayName("findByTenant should return all users for tenant including inactive")
    void findByTenant_shouldReturnAllUsers_whenTenantHasUsers() {
        // Arrange
        ...
        when(principalRepo.find("tenantId = ?1 AND type = ?2", 100L, PrincipalType.USER))
            .thenReturn(mockQuery);

        // Act
        List<Principal> result = service.findByTenant(100L);
    ...
    @Test
    @DisplayName("findByTenant should return empty list when tenant has no users")
    void findByTenant_shouldReturnEmpty_whenTenantHasNoUsers() {
        // Arrange
        io.quarkus.hibernate.orm.panache.PanacheQuery<Principal> mockQuery = mockPanacheQuery(List.of());
        when(principalRepo.find("tenantId = ?1 AND type = ?2", 999L, PrincipalType.USER))
            .thenReturn(mockQuery);

        // Act
        List<Principal> result = service.findByTenant(999L);
    ...
    // ========================================
    // findActiveByTenant TESTS
    // ========================================

    @Test
    @DisplayName("findActiveByTenant should return only active users")
    void findActiveByTenant_shouldReturnOnlyActiveUsers_whenTenantHasUsers() {
        // Arrange
        Principal user1 = createInternalUserPrincipal(1L, "john@acme.com", 100L);
        user1.active = true;

        io.quarkus.hibernate.orm.panache.PanacheQuery<Principal> mockQuery = mockPanacheQuery(List.of(user1));
        when(principalRepo.find("tenantId = ?1 AND type = ?2 AND active = true", 100L, PrincipalType.USER))
            .thenReturn(mockQuery);

        // Act
        List<Principal> result = service.findActiveByTenant(100L);
    ...
    private Principal createInternalUserPrincipal(Long id, String email, Long tenantId) {
    ...
    private Principal createOidcUserPrincipal(Long id, String email, Long tenantId, String externalIdpId) {
        Principal p = new Principal();
        p.id = id;
        p.type = PrincipalType.USER;
        p.tenantId = tenantId;
