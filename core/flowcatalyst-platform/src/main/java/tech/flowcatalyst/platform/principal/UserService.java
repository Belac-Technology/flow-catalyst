...
public class UserService {
     ...
     */
    @Transactional
    public Principal createInternalUser(String email, String password, String name, Long tenantId) {
        // Validate email
     ...
     */
    @Transactional
    public Principal createOrUpdateOidcUser(String email, String name, String externalIdpId, Long tenantId) {
        String emailDomain = extractDomain(email);
     ...
     */
    public List<Principal> findByTenant(Long tenantId) {
        return principalRepo.find("tenantId = ?1 AND type = ?2", tenantId, PrincipalType.USER).list();
    }

    /**
     * Find all active users belonging to a specific tenant.
     *
     * @param tenantId Tenant ID
     * @return List of active principals
     */
    public List<Principal> findActiveByTenant(Long tenantId) {
        return principalRepo.find("tenantId = ?1 AND type = ?2 AND active = true",
            tenantId, PrincipalType.USER).list();
    }
