...
public class ProvisionServiceAccountUseCase {
     ...
     *
     * <p>Includes both the domain event result and the client secret,
     * which is only available at provisioning time.
     *
     ...
     * @param clientSecret The plaintext client secret (null on failure, only available once)
     */
    ...
    public ProvisionResult execute(
        ...
        if (app.serviceAccountPrincipalId != null) {
            return new ProvisionResult(
                Result.failure(new UseCaseError.BusinessRuleViolation(
                    ...
                    Map.of("applicationId", command.applicationId(),
                           "existingPrincipalId", app.serviceAccountPrincipalId)
                )),
                null, null, null
            );
        }
        ...
        // Update application with reference to service account
        app.serviceAccountPrincipalId = principalId;
        ...
        if (result instanceof Result.Success) {
            return new ProvisionResult(result, principal, oauthClient, clientSecret);
        } else {
            return new ProvisionResult(result, null, null, null);
        }
    }
