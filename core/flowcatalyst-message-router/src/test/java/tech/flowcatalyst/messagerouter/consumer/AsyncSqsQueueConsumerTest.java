...
class AsyncSqsQueueConsumerTest {
    ...
    void shouldPollAndProcessMessagesAsynchronously() {
        ...
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            verify(mockQueueManager, atLeastOnce()).routeMessageBatch(anyList());
    ...
    void shouldDeleteMessageOnAsyncAck() {
        ...
        doAnswer(invocation -> {
            List<QueueManager.BatchMessage> batch = invocation.getArgument(0);
            for (QueueManager.BatchMessage batchMsg : batch) {
                // Simulate successful processing - ACK the message
                batchMsg.callback().ack(batchMsg.message());
                routedCount.incrementAndGet();
            }
            return null;
        }).when(mockQueueManager).routeMessageBatch(anyList());
        ...
        doAnswer(invocation -> {
            ...
            for (QueueManager.BatchMessage batchMsg : batch) {
                tech.flowcatalyst.messagerouter.callback.MessageVisibilityControl callback =
                    (tech.flowcatalyst.messagerouter.callback.MessageVisibilityControl) batchMsg.callback();

                // Test fast-fail visibility
                callback.setFastFailVisibility(batchMsg.message());

                // Test reset visibility
                callback.resetVisibilityToDefault(batchMsg.message());
            }
            return null;
        }).when(mockQueueManager).routeMessageBatch(anyList());
