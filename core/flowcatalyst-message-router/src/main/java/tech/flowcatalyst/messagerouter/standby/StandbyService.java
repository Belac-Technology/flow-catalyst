        ...
        try {
            if (isPrimary) {
                ...
                if (lockManager.refreshLock()) {
                    ...
                    // Shut down immediately - let Kubernetes/orchestrator restart us
                    // Standby will take over within 0-30 seconds
                    LOG.severe("CRITICAL: Primary lost lock unexpectedly. Shutting down to allow standby takeover. " +
                              "Container orchestrator will restart this instance.");
                    this.isPrimary = false;
