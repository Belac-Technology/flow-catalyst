...
public class EmbeddedQueueConsumer extends AbstractQueueConsumer {
    ...
    protected void consumeMessages() {
        LOG.infof("Embedded queue consumer started for [%s]", queueName);
        ...
        while (running.get()) {
            ...
            try {
                ...
                // Process message with embedded-specific callback
                processMessage(
                    message.messageJson,
                    message.messageGroupId,  // Pass messageGroupId for FIFO ordering
                ...
                LOG.errorf(e, "Error consuming from embedded queue [%s]", queueName);
                try {
        ...
        LOG.infof("Embedded queue consumer stopped for [%s]", queueName);
    }
    ...
    protected void pollQueueMetrics() {
        while (running.get()) {
            try {
                ...
                try (Connection conn = dataSource.getConnection();
                    ...
                    try (ResultSet rs = stmt.executeQuery()) {
                        if (rs.next()) {
                            ...
                            queueMetrics.recordQueueMetrics(queueName, visibleMessages, invisibleMessages);
                        }
                    }
                }
                ...
                LOG.errorf(e, "Error polling embedded queue metrics for [%s]", queueName);
                try {
                    Thread.sleep(5000);
                } catch (InterruptedException ie) {
                    Thread.currentThread().interrupt();
