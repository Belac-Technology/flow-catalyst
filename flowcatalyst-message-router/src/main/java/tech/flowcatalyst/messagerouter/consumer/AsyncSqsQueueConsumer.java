    ...
    private void submitAsyncPoll() {
        ...
        CompletableFuture<Void> pollFuture = sqsAsyncClient.receiveMessage(receiveRequest)
            .thenAccept(response -> {
                ...
                if (!messages.isEmpty()) {
                    LOG.debugf("Async poll received %d messages, processing immediately", messages.size());
                }
            ...
            .thenRun(() -> {
                ...
                // Chain the next poll if still running
                if (running.get()) {
                    submitAsyncPoll();
                } else {
                    LOG.debug("Consumer stopped, not chaining next poll");
                }
            })
            .exceptionally(ex -> {
                activePollCount.decrementAndGet();
                if (running.get()) {
                    ...
                    LOG.debug("Error during shutdown, exiting cleanly");
                }
                return null;
            });
