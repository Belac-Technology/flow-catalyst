...
pub struct ClientsState {
    ...
    pub application_repo: Option<Arc<crate::repository::ApplicationRepository>>,
    pub application_client_config_repo: Option<Arc<crate::repository::ApplicationClientConfigRepository>>,
    pub audit_service: Option<Arc<crate::service::AuditService>>,
}
...
pub async fn add_note(
    ...
    if let Some(ref audit) = state.audit_service {
        let _ = audit.log_update(&auth.0, "Client", &id, format!("Note added: {}", req.note)).await;
    }
...
pub fn clients_router(state: ClientsState) -> Router {
    Router::new()
        ...
        .route("/by-identifier/:identifier", get(get_client_by_identifier))
        ...
        .route("/:id/applications", get(get_client_applications).put(update_client_applications))
        .route("/:id/applications/:application_id/enable", post(enable_application))
        .route("/:id/applications/:application_id/disable", post(disable_application))
        .with_state(state)
}
