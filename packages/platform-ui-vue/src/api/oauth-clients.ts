...
export const oauthClientsApi = {
  list(params?: { applicationId?: string; active?: boolean }): Promise<OAuthClientListResponse> {
    ...
    return apiFetch(`/admin/platform/oauth-clients${query ? '?' + query : ''}`);
  },

  get(id: string): Promise<OAuthClient> {
    return apiFetch(`/admin/platform/oauth-clients/${id}`);
  },

  getByClientId(clientId: string): Promise<OAuthClient> {
    return apiFetch(`/admin/platform/oauth-clients/by-client-id/${clientId}`);
  },

  create(data: CreateOAuthClientRequest): Promise<CreateOAuthClientResponse> {
    return apiFetch('/admin/platform/oauth-clients', {
      method: 'POST',
  ...
  rotateSecret(id: string): Promise<RotateSecretResponse> {
    return apiFetch(`/admin/platform/oauth-clients/${id}/rotate-secret`, {
      method: 'POST',
    });
  },

  activate(id: string): Promise<{ message: string }> {
    return apiFetch(`/admin/platform/oauth-clients/${id}/activate`, { method: 'POST' });
  },

  deactivate(id: string): Promise<{ message: string }> {
    return apiFetch(`/admin/platform/oauth-clients/${id}/deactivate`, { method: 'POST' });
  },

  delete(id: string): Promise<void> {
    return apiFetch(`/admin/platform/oauth-clients/${id}`, { method: 'DELETE' });
  },
};
